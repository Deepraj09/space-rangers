...
class Invader {
    ...
    shoot(invaderPorjectles) {
        invaderPorjectles.push(new InvaderProjectile({
            position: {
...
class Grid {
  constructor() {
    ...
    this.invaders = []
    const cloumns = Math.floor(Math.random() * 10 + 5)
    const rows = Math.floor(Math.random() * 5 + 2)

    this.width = cloumns * 30

    for(let x = 0; x < cloumns; x++) {
            ...
            projectiles.forEach((projectile, j) => {
                if(projectile.position.y - projectile.radius <= invader.position.y + invader.height && projectile.position.x + projectile.radius >= invader.position.x && projectile.position.x - projectile.radius <= invader.position.x + invader.width && projectile.position.y + projectile.radius >= invader.position.y) {
                    ...
                    setTimeout(() => {
                        ...
                        const indvaderFound = grid.invaders.find(invader2 => 
                         invader2 === invader)
                        const projectileFound = projectiles.find(projectile2 => projectile2 === projectile)
                        
                        // remove invader and projectile 
                        if(indvaderFound && projectileFound){
                            score += 100
                            console.log(score)
